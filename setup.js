 ...
 */
const rewriteFiles = ['public/index.html', 'public/manifest.json', 'src/index.tsx', 'package.json', 'rollup.config.js']
rewriteFiles.forEach(file => {
  const content = fs.readFileSync(fromRoot(file), 'utf-8')
  fs.writeFileSync(fromRoot(file), content.replace(/starter-supertag/g, name), 'utf-8')
})
 ...
 */
const commands = ['git add .', 'git commit -am "first commit from starter-supertag"', 'npm install']
